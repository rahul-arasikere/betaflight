...
static bool sdcard_receiveCID(void)
{
    ...
    sdcard.metadata.productionYear = (((sdinfo->SD_cid.ManufactDate & 0x0F00) >> 8) | ((
                                          sdinfo->SD_cid.ManufactDate & 0xFF) >> 4)) + 2000;
 ...
 */
static sdcardOperationStatus_e sdcardSdio_writeBlock(uint32_t blockIndex, uint8_t *buffer,
 ...
 */
static bool sdcardSdio_readBlock(uint32_t blockIndex, uint8_t *buffer, sdcard_operationCompleteCallback_c callback,
