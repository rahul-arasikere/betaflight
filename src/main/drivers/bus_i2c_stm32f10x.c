...
static void i2c_er_handler(I2CDevice device)
{
    ...
    if (SR1Register & (I2C_SR1_BERR | I2C_SR1_ARLO | I2C_SR1_AF)) {
        ...
        I2C_ITConfig(I2Cx, I2C_IT_BUF,
                     DISABLE);                                // disable the RXNE/TXE interrupt - prevent the ISR tailchaining onto the ER (hopefully)
        if (!(SR1Register & I2C_SR1_ARLO)
            && !(I2Cx->CR1 & I2C_CR1_STOP)) {     // if we dont have an ARLO error, ensure sending of a stop
            if (I2Cx->CR1 &
                I2C_CR1_START) {                                    // We are currently trying to send a start, this is very bad as start, stop will hang the peripheral
    ...
    I2Cx->SR1 &= ~(I2C_SR1_BERR | I2C_SR1_ARLO | I2C_SR1_AF |
                   I2C_SR1_OVR);     // reset all the error bits to clear the interrupt
    state->busy = 0;
}
...
void i2c_ev_handler(I2CDevice device)
{
    ...
    static uint8_t subaddress_sent,
           final_stop;                                 // flag to indicate if subaddess sent, flag to indicate final bus condition
    static int8_t index;                                                        // index is signed -1 == send the subaddress
    uint8_t SReg_1 = I2Cx->SR1;                                                 // read the status register here

    if (SReg_1 & I2C_SR1_SB) {                                                  // we just sent a start - EV5 in ref manual
        I2Cx->CR1 &=
            ~I2C_CR1_POS;                                              // reset the POS bit so ACK/NACK applied to the current byte
        ...
        if (state->reading && (subaddress_sent || 0xFF == state->reg)) {          // we have sent the subaddr
            ...
            if (state->bytes == 2)
                I2Cx->CR1 |=
                    I2C_CR1_POS;                                       // set the POS bit so NACK applied to the final byte in the two byte read
            ...
            if (state->reg !=
               ...
               I2C_SR1_ADDR) {                                       // we just sent the address - EV6 in ref manual
        ...
        if (index != -1) {                                              // we dont have a subaddress to send
            ...
            if (state->reading
                || !(state->bytes))                                      // if receiving or sending 0 bytes, flush now
                I2C_ITConfig(I2Cx, I2C_IT_BUF, DISABLE);                // disable TXE to allow the buffer to flush
        }
    }
...
void i2cInit(I2CDevice device)
{
    ...
    I2C_ITConfig(I2Cx, I2C_IT_EVT | I2C_IT_ERR,
                 DISABLE);               // Disable EVT and ERR interrupts - they are enabled by the first request
